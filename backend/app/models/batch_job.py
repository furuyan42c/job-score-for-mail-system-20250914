...
class BatchJob(Base):
    ...
    def fail(self, error_message: str) -> None:
        ...
        if self.should_retry():
            ...
            logger.warning(
                f"Job {self.job_name} failed (attempt {self.retry_count}/{self.max_retries}): {error_message}"
            )
        else:
            self.status = JobStatus.FAILED
            self.completed_at = datetime.utcnow()
            logger.error(
                f"Job {self.job_name} permanently failed after {self.retry_count} attempts: {error_message}"
            )
    ...
    async def cleanup_old_jobs(
        cls, db_session: AsyncSession, days_to_keep: int = CLEANUP_DAYS_DEFAULT
    ) -> int:
    ...
    @classmethod
    async def can_start_new_job(
        cls, db_session, job_type: JobType, max_concurrent: int = 3
    ) -> bool:
